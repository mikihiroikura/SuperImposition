...
realsense::realsense(string num, rs2_format colorformat, unsigned int color_width, unsigned int color_height, unsigned int color_fps, rs2_format depthformat, unsigned int depth_width, unsigned int depth_height, unsigned int depth_fps) {
	...
	realsense::serial_num = num;
	config.enable_device(realsense::serial_num);
	realsense::color_width = color_width;
	realsense::color_height = color_height;
	realsense::color_fps = color_fps;
	realsense::depth_width = depth_width;
	realsense::depth_height = depth_height;
	realsense::depth_fps = depth_fps;
	config.enable_stream(RS2_STREAM_COLOR, realsense::color_width, realsense::color_height, colorformat, realsense::color_fps);
	config.enable_stream(RS2_STREAM_DEPTH, realsense::depth_width, realsense::depth_height, depthformat, realsense::depth_fps);
