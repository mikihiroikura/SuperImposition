...
struct rect
{
    ...
    rect adjust_ratio(flow2_gl size) const
    {
...
extern inline void draw_text(int x, int y, const char* text);

extern void set_viewport(const rect& r);
...
private:
    ...
    flow2_gl xyz_to_xy(float x, float y, float z, GLfloat model[], GLfloat proj[], float vec_norm)
    {
    ...
    static void draw_circle(float xx, float xy, float xz, float yx, float yy, float yz, float radius = 1.1, flow3_gl center = { 0.0, 0.0, 0.0 }, float intensity = 0.5f)
    {
        const auto N = 50;
...
private:
    GLFWwindow * win;
    std::map<int, texture_gl> _textures;
...
struct glfw_state {
    glfw_state(float pitch = 15.0, float roll = 15.0) : pitch(pitch), roll(roll), last_x(0.0), last_y(0.0),
